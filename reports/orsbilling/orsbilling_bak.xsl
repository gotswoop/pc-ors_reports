        ...
        <html class="no-js" lang="en" >
			...
			<body>
              ...
              <li class="tab-title"><a href="#setuppanel" onclick="loadXMLDoc();">Setup</a></li>
                ...
                <xsl:apply-templates select="/records/item[routing_users = ',lstewart2,lmerritt,'] | 
                /records/item[routing2_users = ',lstewart2,lmerritt,'] |
                /records/item[routing3_users = ',lstewart2,lmerritt,'] |
                /records/item[routing4_users = ',lstewart2,lmerritt,'] |
                /records/item[routing5_users = ',lstewart2,lmerritt,'] |
                /records/item[routing6_users = ',lstewart2,lmerritt,'] |
                /records/item[routing7_users = ',lstewart2,lmerritt,'] |
                /records/item[routing8_users = ',lstewart2,lmerritt,'] |
                /records/item[routing9_users = ',lstewart2,lmerritt,'] |
                /records/item[routing10_users = ',lstewart2,lmerritt,'] " mode="personal">
                <xsl:sort select="translate(translate(patient_last, 'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),' ','')" order="ascending" />
                        </xsl:apply-templates>
                </fieldset>
                </xsl:if>
              ...
              <!--  <xsl:if test="/records/item[routing!='' and contains(routing_record_users,concat(',',/records/@username,',')) and routing_record_users != concat(',',/records/@username,',INCOMPLETE,') and routing_record_users != concat(',',/records/@username,',COMPLETE,')]"> -->
                ...
                <legend><xsl:value-of select="/records/@username"/></legend>
                <xsl:apply-templates select="/records/item[routing_users = (concat(',',/records/@username,','))] | 
                /records/item[routing2_users = (concat(',',/records/@username,','))] |
                /records/item[routing3_users = (concat(',',/records/@username,','))] |
                /records/item[routing4_users = (concat(',',/records/@username,','))] |
                /records/item[routing5_users = (concat(',',/records/@username,','))] |
                /records/item[routing6_users = (concat(',',/records/@username,','))] |
                /records/item[routing7_users = (concat(',',/records/@username,','))] |
                /records/item[routing8_users = (concat(',',/records/@username,','))] |
                /records/item[routing9_users = (concat(',',/records/@username,','))] |
                /records/item[routing10_users = (concat(',',/records/@username,','))] " mode="personal">
                <xsl:sort select="translate(translate(patient_last, 'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),' ','')" order="ascending" />
                        </xsl:apply-templates>
                </fieldset>
                </xsl:if>
                ...
                <fieldset style="background-color:#F5E5CB;">
                <legend><xsl:value-of select="/records/@username"/></legend>
                <xsl:apply-templates select="/records/item[contains(routing_record_users,concat(',',/records/@username,','))]" mode="personal">
                <xsl:sort select="translate(translate(patient_last, 'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),' ','')" order="ascending" />
                        </xsl:apply-templates>
                </fieldset>
                </xsl:if>
                
                <xsl:if test="not (/records/item[contains(routing_record_users,concat(',',/records/@username,','))])">
        				    No records have been routed to you.
        				</xsl:if>
            </form>
    				...
    				<div class="content" id="alluserspanel">
                        <form>
                        <xsl:apply-templates select="/records/item[contains(routing_record_users,',INCOMPLETE,')]" mode="incomplete">
                        <xsl:sort select="patient_last" order="ascending" />
                        </xsl:apply-templates>                        
                        </form>
    				</div>
...
<xsl:if test="((/records/@username = 'lmerritt') and (((contains(billing_surgeon,'Achor') or contains(billing_surgeon,'Burgess') or contains(billing_surgeon,'Choo') or contains(billing_surgeon,'Harvin') or contains(billing_surgeon,'Bonner')) and (contains(routing_users,',lmerritt,')))
                                                      or ((contains(billing_surgeon2,'Achor') or contains(billing_surgeon2,'Burgess') or contains(billing_surgeon2,'Choo')or contains(billing_surgeon2,'Harvin')or contains(billing_surgeon2,'Bonner')) and (contains(routing2_users,',lmerritt,')))
                                                      or ((contains(billing_surgeon3,'Achor') or contains(billing_surgeon3,'Burgess') or contains(billing_surgeon3,'Choo')or contains(billing_surgeon3,'Harvin')or contains(billing_surgeon3,'Bonner')) and (contains(routing3_users,',lmerritt,')))
                                                      or ((contains(billing_surgeon4,'Achor') or contains(billing_surgeon4,'Burgess') or contains(billing_surgeon4,'Choo')or contains(billing_surgeon4,'Harvin')or contains(billing_surgeon4,'Bonner')) and (contains(routing4_users,',lmerritt,')))
                                                      or ((contains(billing_surgeon5,'Achor') or contains(billing_surgeon5,'Burgess') or contains(billing_surgeon5,'Choo')or contains(billing_surgeon5,'Harvin')or contains(billing_surgeon5,'Bonner')) and (contains(routing5_users,',lmerritt,')))
                                                      or ((contains(billing_surgeon6,'Achor') or contains(billing_surgeon6,'Burgess') or contains(billing_surgeon6,'Choo')or contains(billing_surgeon6,'Harvin')or contains(billing_surgeon6,'Bonner')) and (contains(routing6_users,',lmerritt,')))
                                                      or ((contains(billing_surgeon7,'Achor') or contains(billing_surgeon7,'Burgess') or contains(billing_surgeon7,'Choo')or contains(billing_surgeon7,'Harvin')or contains(billing_surgeon7,'Bonner')) and (contains(routing7_users,',lmerritt,')))
                                                      or ((contains(billing_surgeon8,'Achor') or contains(billing_surgeon8,'Burgess') or contains(billing_surgeon8,'Choo')or contains(billing_surgeon8,'Harvin')or contains(billing_surgeon8,'Bonner')) and (contains(routing8_users,',lmerritt,')))
                                                      or ((contains(billing_surgeon9,'Achor') or contains(billing_surgeon9,'Burgess') or contains(billing_surgeon9,'Choo')or contains(billing_surgeon9,'Harvin')or contains(billing_surgeon9,'Bonner')) and (contains(routing9_users,',lmerritt,')))
                                                      or ((contains(billing_surgeon10,'Achor') or contains(billing_surgeon10,'Burgess') or contains(billing_surgeon10,'Choo') or contains(billing_surgeon10,'Harvin')or contains(billing_surgeon10,'Bonner')) and (contains(routing10_users,',lmerritt,'))))) or
                ((/records/@username = 'lstewart2') 
                          and (((contains(billing_surgeon,'Gary') or contains(billing_surgeon,'Dodwad') or contains(billing_surgeon,'Munz') or  contains(billing_surgeon,'Woerner') or contains(billing_surgeon,'Prasarn') or contains(billing_surgeon,'Routt') or contains(billing_surgeon,'Tiffany')or contains(billing_surgeon,'Younas')) and (contains(routing_users,',lstewart2,')))
                          or ((contains(billing_surgeon2,'Gary') or contains(billing_surgeon2,'Dodwad') or contains(billing_surgeon2,'Munz') or  contains(billing_surgeon2,'Woerner') or contains(billing_surgeon2,'Prasarn') or contains(billing_surgeon2,'Routt') or contains(billing_surgeon2,'Tiffany')or contains(billing_surgeon2,'Younas')) and (contains(routing2_users,',lstewart2,')))
                          or ((contains(billing_surgeon3,'Gary') or contains(billing_surgeon3,'Dodwad') or contains(billing_surgeon3,'Munz') or  contains(billing_surgeon3,'Woerner') or contains(billing_surgeon3,'Prasarn') or contains(billing_surgeon3,'Routt') or contains(billing_surgeon3,'Tiffany') or contains(billing_surgeon3,'Younas')) and (contains(routing3_users,',lstewart2,')))
                          or ((contains(billing_surgeon4,'Gary') or contains(billing_surgeon4,'Dodwad') or contains(billing_surgeon4,'Munz') or  contains(billing_surgeon4,'Woerner') or contains(billing_surgeon4,'Prasarn') or contains(billing_surgeon4,'Routt') or contains(billing_surgeon4,'Tiffany')or contains(billing_surgeon4,'Younas')) and (contains(routing4_users,',lstewart2,')))
                          or ((contains(billing_surgeon5,'Gary') or contains(billing_surgeon5,'Dodwad') or contains(billing_surgeon5,'Munz') or  contains(billing_surgeon5,'Woerner') or contains(billing_surgeon5,'Prasarn') or contains(billing_surgeon5,'Routt') or contains(billing_surgeon5,'Tiffany')or contains(billing_surgeon5,'Younas')) and (contains(routing5_users,',lstewart2,')))
                          or ((contains(billing_surgeon6,'Gary') or contains(billing_surgeon6,'Dodwad') or contains(billing_surgeon6,'Munz') or  contains(billing_surgeon6,'Woerner') or contains(billing_surgeon6,'Prasarn') or contains(billing_surgeon6,'Routt') or contains(billing_surgeon6,'Tiffany')or contains(billing_surgeon6,'Younas')) and (contains(routing6_users,',lstewart2,')))
                          or ((contains(billing_surgeon7,'Gary') or contains(billing_surgeon7,'Dodwad') or contains(billing_surgeon7,'Munz') or  contains(billing_surgeon7,'Woerner') or contains(billing_surgeon7,'Prasarn') or contains(billing_surgeon7,'Routt') or contains(billing_surgeon7,'Tiffany')or contains(billing_surgeon7,'Younas')) and (contains(routing7_users,',lstewart2,')))
                          or ((contains(billing_surgeon8,'Gary') or contains(billing_surgeon8,'Dodwad') or contains(billing_surgeon8,'Munz') or  contains(billing_surgeon8,'Woerner') or contains(billing_surgeon8,'Prasarn') or contains(billing_surgeon8,'Routt') or contains(billing_surgeon8,'Tiffany')or contains(billing_surgeon8,'Younas')) and (contains(routing8_users,',lstewart2,')))
                          or ((contains(billing_surgeon9,'Gary') or contains(billing_surgeon9,'Dodwad') or contains(billing_surgeon9,'Munz') or  contains(billing_surgeon9,'Woerner') or contains(billing_surgeon9,'Prasarn') or contains(billing_surgeon9,'Routt') or contains(billing_surgeon9,'Tiffany')or contains(billing_surgeon9,'Younas')) and (contains(routing9_users,',lstewart2,')))
                          or ((contains(billing_surgeon10,'Gary') or contains(billing_surgeon10,'Dodwad') or contains(billing_surgeon10,'Munz') or  contains(billing_surgeon10,'Woerner') or contains(billing_surgeon10,'Prasarn') or contains(billing_surgeon10,'Routt') or contains(billing_surgeon10,'Tiffany')or contains(billing_surgeon10,'Younas')) and (contains(routing10_users,',lstewart2,')))))
                          or (/records/@username != 'lstewart2' and /records/@username != 'lmerritt')"> 
